...
- Restore/reconcile on add: the real auth flag wins over restored state; an expiry that passed while HA was down revokes at boot (fail closed); an externally-flipped `local_only=False` is adopted and given a fresh timer (never open-ended). Mid-runtime external flips are reconciled live via `EVENT_USER_UPDATED` (from `homeassistant.auth.const`, as is `EVENT_USER_REMOVED` — neither lives in `homeassistant.const`); our own auth writes re-fire that event, so the handler is deliberately idempotent.
