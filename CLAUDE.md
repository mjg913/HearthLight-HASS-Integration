...
5. Remote access: optional support-user provisioning (`provision_support_user`, default off) via `support_user.py`, then `_async_cleanup_stale_users` (fail-closed device removal for de-selected users; a stale support key deletes the account itself), builds `entry.runtime_data` (`dict[registry_key, RemoteAccessData]`, see `data.py`), then forwards to the `switch` + `number` platforms. Always-on — the default `managed_users: []` just yields zero entities.
