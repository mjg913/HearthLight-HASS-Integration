...
Single vanilla-JS file, no build step, one Lovelace resource URL — **new cards go in this same file** (a second file would need new static-path + resource plumbing). Custom elements: `hearthlight-brand` + editor, `hearthlight-remote-access` + editor, `hearthlight-navbar` (no editor), `hearthlight-contact` + editor, and the dashboard strategy `ll-strategy-dashboard-hearthlight` (plain class, statics only, invoked as `generate(config, hass)`; also pushed to `window.customStrategies` for the HA 2026.5+ new-dashboard dialog). The strategy builds Home/Spaces/System/Support sections views from `hass.areas/devices/entities`, filtering anything labeled `hearthlight-exclude` (matched by label **id**, i.e. name-slug) or registry-hidden, and injects the navbar into every view. A module-level `initAccessPill()` owns the floating "support access active" pill (shown while any `hearthlight` switch is on; tap → Support view; full remote-access card lives only on the Support view). It shows on Lovelace panels only, top-left of the content surface — anchored by measuring `hui-root`'s rect (sidebar width) and the position:fixed header's rect (viewport-stable under scroll, since the window is HA's scroll container), so kiosk-mode hiding the header/sidebar slides it toward the viewport corner. Placement is always measured through open shadow roots, never injected into HA's DOM. In-header placement on Settings/Profile was tried (v0.6.6–0.6.8) and reverted — their toolbar internals vary too much to anchor against reliably. `hearthlight-contact` opens `tel:`/`mailto:` on mobile UAs and copies-with-inline-confirmation elsewhere. The remote-access card re-renders in `set hass` (diffed on its watched switch/number state objects), runs a 1 s countdown interval only while a window is active (cleared in `disconnectedCallback`), and derives the duration number from the switch via `hass.entities[...].device_id`. How the brand card's theme adaptation works — and the contract that makes it safe:
